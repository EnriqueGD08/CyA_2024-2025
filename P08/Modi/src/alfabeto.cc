...
Alfabeto::Alfabeto(const std::string& alfabeto) {
  ...
  for (int i = 0; i < alfabeto.size(); i++) {
    ...
    if (alfabeto[i] == '&') {
      std::cout << "El simbolo " << alfabeto[i] << " no es un simbolo válido." << std::endl;
      std::cout << "En el alfabeto no puede estar ni '&' ni letras en mayúscula." << std::endl;
      continue;
    }
 ...
 * @param alfabeto Conjunto de símbolos que forman el alfabeto
 */
void Alfabeto::InsertarAlfabeto(const char alfabeto) {
  alfabeto_.insert(alfabeto);
}
...
std::istream& operator>>(std::istream& is, Alfabeto& alfabeto) {
  ...
  for (int i = 0; i < alfabeto.GetAlfabeto().size(); i++) {
    if (auxiliar[i] == ' ') {
      ...
      alfabeto.InsertarAlfabeto(auxiliar[i]);
    }
  } 
  return is;
}